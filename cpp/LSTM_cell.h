...
// Define USE_OPENMP to enable OpenMP
#define USE_OPENMP
...
public:
    ...
    std::vector<std::vector<double>> forward(const std::vector<std::vector<double>>& inputs) {
        ...
        for (size_t q = 0; q < inputs.size(); ++q) {
            ...
            cell_states[q] = elementWiseAdd(elementWiseMultiply(forget_gates[q], cell_states[q - 1]), elementWiseMultiply(input_gates[q], candidate_gates[q]));

            hidden_states[q] = elementWiseMultiply(output_gates[q], tanh_vector(cell_states[q]));
    ...
    std::vector<std::vector<double>> getConcatInputs() {
        std::vector<std::vector<double>> concat_inputs_matrix;
        for (auto input : concat_inputs) {
            concat_inputs_matrix.push_back(input.second);
        }
        return concat_inputs_matrix;
    }
    ...
    void printOrigins(int epoch) const {
        std::cout << epoch << ", "
                  << wf[0][0] << ", " << bf[0] << ", "
                  << wi[0][0] << ", " << bi[0] << ", "
                  << wc[0][0] << ", " << bc[0] << ", "
    ...
    void train(const std::vector<std::vector<double>>& xtrain, const std::vector<std::vector<double>>& ytrain) {
        ...
        for (i = 0; i < num_epochs; i++) {
            predictions = forward(xtrain);
            errors.clear();
            for (j = 0; j < predictions.size(); j++) {
                error_row.clear();
                for (k = 0; k < predictions[0].size(); k++) {
                    error_row.push_back(ytrain[j][k] - predictions[j][k]);
                }
                errors.push_back(error_row);
            }
            std::cout << "Epoch: " << i + 1 << " Error: " << absSum2DVector(errors) << std::endl;
